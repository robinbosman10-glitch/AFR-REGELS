(()=>{'use strict';
  ...
  function renderManagedArticles(){const box=$('#managedArticlesList'),query=$('#managedArticleSearch').value.trim().toLowerCase();box.textContent='';const shown=managedArticles.filter(item=>(item.title+' '+(articlePageNames[item.page]||item.page)).toLowerCase().includes(query));shown.forEach(item=>{const row=document.createElement('article');row.className='managed-article-row';const copy=document.createElement('div'),title=document.createElement('strong'),page=document.createElement('small');title.textContent=item.title;page.textContent=articlePageNames[item.page]||item.page;copy.append(title,page);row.append(copy);if(profile?.role==='owner'){const remove=document.createElement('button');remove.type='button';remove.className='owner-delete-article';remove.textContent='Verwijderen';remove.addEventListener('click',()=>deleteManagedArticle(item,remove));row.append(remove)}box.append(row)});if(!shown.length)box.textContent=query?'Geen artikelen gevonden.':'Geen artikelen gevonden.'}
